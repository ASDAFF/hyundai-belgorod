...
if(CModule::IncludeModule("iblock")):
    ...
    $xml = file_get_contents($_SERVER['DOCUMENT_ROOT'].'/XML_upload_for_1c/oskol/used_car.xml',true);
    $files = scandir($_SERVER['DOCUMENT_ROOT'].'/XML_upload_for_1c/oskol/used');
    ...
    foreach($xml->ContractList->Contract as $cont){
        ...
        if (in_array((string)$cont->VIN, $files)) {
            $dir_img = array_diff( scandir($_SERVER['DOCUMENT_ROOT'].'/XML_upload_for_1c/oskol/used/'.(string)$cont->VIN),array('.','..'));
            foreach($dir_img as $img){
                $img_path = $_SERVER['DOCUMENT_ROOT'].'/XML_upload_for_1c/oskol/used/'.(string)$cont->VIN.'/'.$img;
                if(getimagesize($img_path)[0] > 600) {
                    resize($img_path, 550);
                }
                $PROP['SLIDER'][] = '/XML_upload_for_1c/oskol/used/'.(string)$cont->VIN.'/'.$img;
            }
            //var_dump('ok');
        }else{
    ...
    $ok =  CEvent::Send("B_OSK_AUTO", SITE_ID, $arEventFields);
