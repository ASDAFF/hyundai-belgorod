...
$propSection = getPropertySection(11,$arResult['SECTION']['PATH'][0]['ID'],array('UF_SERVICE_LEFT_MENU','UF_ENABLE_SLIDER','UF_DOCUMENT','UF_SERVICE_TABS','UF_ENABLE_VIN','UF_SERVICE_CALC','UF_SLIDER_SERVICE','UF_BIG_SLIDER'));
?>
...
<div class="container" style="margin-top: 0px">
	...
	$APPLICATION->IncludeComponent(
		...
		array(
			...
			"SORT_ORDER1" => "DESC",
			"SORT_ORDER2" => "ASC",
			"COMPONENT_TEMPLATE" => "litle-slider"
		),
		false
	);
	endif;
	...
	<?if($propSection['UF_DOCUMENT'] and $APPLICATION->GetCurPage() != '/service/'):?>
		<div class="service-document-block">
			...
			<div class="avaible-documents" data-model="" style="display: block;">

				<div class="avaible-documents-list grid">
					<?
					foreach($propSection['UF_DOCUMENT'] as $key=>$value):
						?>
					<div class="avaible-document-wrap grid-el grid-el-33">
						<?
						print tplvar('document_name_'.$key, true,false,1);
						print tplvar('document_model_'.$key, true,false,1);
						?>
						<a target="_blank" href="<?=CFile::GetPath($value);?>" class="avaible-document avaible-document--manual">
							<div class="avaible-document-title"><?= tplvar('document_name_'.$key);?></div>
							<div class="avaible-document-model"><?= tplvar('document_model_'.$key);?></div>
						</a>
					</div>
					<?endforeach;?>
				</div>
	...
	<?
	if($propSection['UF_SERVICE_LEFT_MENU'] == null):
	?>
