...
>>>>>>> 65e2384e4b50b2e771d9064744d8b8a0403576fc
	...
	$(document).on('click', '.changemonth', function(e){
		...
		$.post('/requestnew/getCalendar', 'month=' + mth, function(res){
			$('#calendar').replaceWith(res);
			
			// Следующее действие - получить мероприятия за месяц
			$.post('/requestnew/getEvents', 'month=' + mth + '&html=1', function(res){
				$('.events-center-body-content-wrap').html('');
				$('.events-center-body-content-wrap').html(res);
			});
		});
	});
	...
	$(document).on('click', '.changeday', function(e){
		...
		$('.datepicker-body-item--active').removeClass('datepicker-body-item--active').addClass('datepicker-body-item--prev');
		
		$(this).removeClass('datepicker-body-item--prev');
		$(this).addClass('datepicker-body-item--active');
		
		$.post('/requestnew/getEvents', 'month=' + mth + '&day=' + day + '&html=1', function(res){
			$('.events-center-body-content-wrap').html('');
			$('.events-center-body-content-wrap').html(res);
		});
	});
	
	$(document).on('click', '.models-open', function(e){
		$("html, body").animate({ scrollTop: 0 }, 300, 'easeOutCirc');
		
		//console.log($('#gnbShowroom').css('visibility'));
		if($('#gnbShowroom').css('visibility') == 'hidden')
		{
			$('.showroom-open').trigger('click');
		}
	});
	// Обработчик клика по дате
	
	// Открываем видео-попап
	$(document).on('click', '.video_link', function(e){
		e.preventDefault();
		$(".videoFrame").attr("src", $(this).attr('data-src'));
		$(".videoPopup").fadeIn();
	});

	// Закрываем видео-попап :)
